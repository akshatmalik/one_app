...
import { useSessionTimer, formatElapsed, roundToLogHours } from './hooks/useSessionTimer';
...
export default function GameAnalyticsPage() {
  ...
  // Live session timer
  const { session: liveSession, elapsed: liveElapsed, isRunning: timerRunning, start: timerStart, stop: timerStop, abandon: timerAbandon } = useSessionTimer();

  const handleStartSession = useCallback((game: GameWithMetrics) => {
    if (timerRunning && liveSession?.gameId === game.id) return;
    if (timerRunning) {
      const ok = window.confirm(`Stop current session for "${liveSession?.gameName}" and start one for "${game.name}"?`);
      if (!ok) return;
      timerAbandon();
    }
    timerStart(game.id, game.name, game.thumbnail);
    showToast(`⏱ Session started for ${game.name}`, 'success');
  }, [timerRunning, liveSession, timerStart, timerAbandon, showToast]);

  const handleStopSession = useCallback(async (mood?: PlayLog['mood'], notes?: string) => {
    const result = timerStop();
    if (!result) return;
    const { hoursRaw, session: completed } = result;
    const roundedHours = roundToLogHours(hoursRaw * 3600);
    const game = games.find(g => g.id === completed.gameId);
    if (!game) { showToast(`Logged ${roundedHours}h`, 'success'); return; }
    const now = new Date();
    ...
    showToast(`✓ ${roundedHours}h logged for ${completed.gameName}`, 'success');
  }, [timerStop, games, updateGame, showToast]);
  ...
  return (
    <div className="min-h-[calc(100vh-60px)] flex flex-col">
            ...
            <>
              {games.length === 0 ? (
                ...
                <GameCardList
                  ...
                  activeSessionGameId={liveSession?.gameId}
                />
              )}
            </>
          )}
      ...
      {/* Live Session Timer — floating widget */}
      {timerRunning && liveSession && (
        <LiveSessionTimer
          session={liveSession}
          elapsed={liveElapsed}
          onStop={handleStopSession}
          onAbandon={timerAbandon}
        />
      )}
    </div>
  );
            ...
            <div className="space-y-3">
              {nowPlayingGames.map(g => (
                <div key={g.id} className={`game-card-animate${enteringCards.has(g.id) ? ' game-card-enter' : ''}`}>
                  <NowPlayingCard game={g} allGames={allGames} onClick={() => onCardClick(g)} onQuickLog={(h) => onQuickLog(g, h)} sortBy={sortBy} tintColor={gameColors.get(g.id)} eloRanking={eloByGameId.get(g.id)} gameTier={tierAssignments[g.id] as GameTier | undefined} eloTierRank={eloTierRanks.get(g.id)} onStartSession={() => onStartSession(g)} isActiveSession={activeSessionGameId === g.id} />
                </div>
              ))}
            </div>
          </div>
      ...
      {nowPlayingGames.length > 0 && (
        <div>
          ...
          <div className="space-y-3">
            {nowPlayingGames.map(g => (
              <div key={g.id} className={`game-card-animate${enteringCards.has(g.id) ? ' game-card-enter' : ''}`}>
                <NowPlayingCard game={g} allGames={allGames} onClick={() => onCardClick(g)} onQuickLog={(h) => onQuickLog(g, h)} eloRanking={eloByGameId.get(g.id)} gameTier={tierAssignments[g.id] as GameTier | undefined} onStartSession={() => onStartSession(g)} isActiveSession={activeSessionGameId === g.id} />
              </div>
            ))}
          </div>
        </div>
...
function NowPlayingCard({ game, allGames, onClick, onQuickLog, sortBy = 'hours', tintColor, eloRanking, gameTier, eloTierRank, onStartSession, isActiveSession }: {
          ...
          <div className="px-3 pb-2.5 pt-1 flex items-center gap-1.5">
            ...
            {onStartSession && (
              <button
                ...
                title={isActiveSession ? 'Timer running' : 'Start live timer'}
                className={clsx(
                  ...
                  isActiveSession
                    ...
                    : 'bg-purple-600/20 text-purple-400 hover:bg-purple-600/30 active:scale-95'
                )}
              >
                {isActiveSession ? <Timer size={12} /> : <Play size={12} className="translate-x-px" />}
              </button>
            )}
          </div>
...
function PosterCard({ game, allGames, idx, onClick, onQuickLog, isInQueue, sortBy = 'hours', tintColor, aiQuip, eloRanking, gameTier, eloTierRank, onStartSession, isActiveSession }: {
  game: GameWithMetrics;
  allGames: Game[];
  idx: number;
  onClick: () => void;
      ...
      <div className={clsx('card-flip-inner', isFlipped && 'flipped')}>
        ...
        >
          ...
          <div className="px-3 py-2.5 flex items-center gap-3">
            ...
            {momentum.length >= 2 && <MomentumDots sessions={momentum} />}
            {game.platform && <span className="text-[9px] px-1.5 py-0.5 bg-white/5 rounded text-white/30">{game.platform}</span>}
            {/* Start timer button — only for playable games */}
            {onStartSession && game.status !== 'Completed' && game.status !== 'Wishlist' && (
              <button
                onClick={(e) => { e.stopPropagation(); onStartSession(); }}
                title={isActiveSession ? 'Timer running' : 'Start live timer'}
                className={clsx(
                  ...
                  isActiveSession
                    ? 'bg-green-500/20 text-green-400 health-pulse-medium'
                    : 'bg-white/5 text-white/30 hover:bg-purple-600/20 hover:text-purple-400 active:scale-95'
                )}
              >
                {isActiveSession ? <Timer size={10} /> : <Play size={10} className="translate-x-px" />}
              </button>
            )}
          </div>
...
function CompactCard({ game, allGames, idx, onClick, onLogTime, onToggleQueue, onDelete, isInQueue, sortBy = 'hours', tintColor, aiQuip, eloRanking, gameTier, eloTierRank, onStartSession, isActiveSession }: {
  game: GameWithMetrics;
  allGames: Game[];
  idx: number;
  onClick: () => void;
          ...
          <div className="flex items-center gap-1.5 pt-2 border-t border-white/5">
            ...
            {onStartSession && game.status !== 'Completed' && game.status !== 'Wishlist' && (
              <button
                ...
                title={isActiveSession ? 'Timer running' : 'Start live timer'}
                className={clsx(
                  'flex items-center gap-1 px-2.5 py-1.5 rounded-lg transition-all text-xs font-medium',
                  isActiveSession
                    ? 'text-green-400 bg-green-500/10 health-pulse-fast'
                    : 'text-purple-400/70 hover:text-purple-300 hover:bg-purple-500/10 active:scale-95'
                )}
              >
                {isActiveSession ? <Timer size={12} /> : <Play size={12} className="translate-x-px" />}
                {isActiveSession ? 'Live' : 'Timer'}
              </button>
            )}
            <button
