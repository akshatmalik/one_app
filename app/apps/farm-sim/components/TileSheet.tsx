      ...
      <fieldset disabled={paused} className="space-y-2.5 p-3 disabled:opacity-60">
        ...
        {!inRange ? (
          ...
          <div className="grid grid-cols-2 gap-1.5">
            ...
            {actions.includes('buildExtractor') ? <button className={actionClass} disabled={state.facilities.workshop.level < 2 || state.gold < EXTRACTOR_BUILD_COST.gold || state.items.bricks < EXTRACTOR_BUILD_COST.bricks || state.items.machineParts < EXTRACTOR_BUILD_COST.machineParts} onClick={() => run({ type: 'buildExtractor', idx })}><Hammer size={16} /> Automate · {EXTRACTOR_BUILD_COST.gold}g</button> : null}
