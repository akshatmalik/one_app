...
- **Save for later + caching**: bookmark action parks a game as "Someday" in the panel only (no library entry). Confirmed monthly lineups are cached in localStorage (`getCachedDrops`/`clearDropsCache`, 1yr) keyed by tier+month, so re-sync/backfill never re-searches a finalized month; empty pulls aren't cached (stay retryable).
- **10 enhancements**: (1) "Don't buy it — it's free" cross-check vs Buy Queue + Wishlist with potential-savings + remove-from-buy-queue; (2) subscription ROI meter (claimed value vs `SUBSCRIPTION_ANNUAL_COST` for the tier, this-year + all-time); (3) Pick of the Month hero card; (4) Leaving Soon (search now returns a `leaving` bucket; owned matches flagged "play before it's gone"); (5) Reclaim owned freebies (retro-tag `acquiredFree`/`subscriptionSource` to credit savings via `onUpdateGame`); (6) sort (match/metacritic/newest) + bucket filter; (7) "Claim by <date>" on monthly cards (`monthlyClaimDeadline`) + "Claim all monthly"; (8) taste-aware drop insight line; (9) expandable Sources/receipts; (10) new-drop dot on the Discover tab.
