import { Encounter, EnemyType, StageCondition } from './types';

// ── THREAT SCALING ──────────────────────────────────────────────────────

/**
 * Get threat level based on current day.
 * Day 1-3: TL1 (60% stats), Day 4-6: TL2 (80%), Day 7-10: TL3 (100%),
 * Day 11-15: TL4 (120%), Day 16-20: TL5 (140% + bonus enemy),
 * Day 21+: TL6 (160% + 2 bonus enemies + stage conditions)
 */
export function getThreatLevel(day: number): number {
  return Math.min(6, Math.floor(day / 3.5) + 1);
}

const THREAT_HP_MULTIPLIERS: Record<number, number> = {
  1: 0.6, 2: 0.8, 3: 1.0, 4: 1.2, 5: 1.4, 6: 1.6,
};
const THREAT_ATK_MULTIPLIERS: Record<number, number> = {
  1: 0.6, 2: 0.8, 3: 1.0, 4: 1.2, 5: 1.4, 6: 1.6,
};

/**
 * Scale an encounter's enemies to the given threat level.
 * At TL5+ adds a bonus enemy. At TL6 adds two bonus enemies.
 * Stage conditions roll in at TL3+ (day 5+ equivalent) with 25% chance.
 * Two conditions can stack at TL5+.
 */
export function scaleEncounterToThreat(encounter: Encounter, threatLevel: number): Encounter {
  const hpMult = THREAT_HP_MULTIPLIERS[threatLevel] ?? 1.0;
  const atkMult = THREAT_ATK_MULTIPLIERS[threatLevel] ?? 1.0;

  const scaledEnemies = (encounter.enemies ?? []).map(enemy => ({
    ...enemy,
    health: Math.round(enemy.health * hpMult),
    maxHealth: Math.round(enemy.maxHealth * hpMult),
    damage: Math.round(enemy.damage * atkMult),
  }));

  // TL5+: add a bonus runner enemy
  if (threatLevel >= 5 && scaledEnemies.length > 0) {
    const baseEnemy = scaledEnemies[0];
    scaledEnemies.push({
      name: 'Raider Scout',
      health: Math.round(12 * hpMult),
      maxHealth: Math.round(12 * hpMult),
      damage: Math.round(6 * atkMult),
      defense: baseEnemy.defense,
    });
  }
  // TL6: add a second bonus enemy
  if (threatLevel >= 6 && scaledEnemies.length > 0) {
    scaledEnemies.push({
      name: 'Heavy Raider',
      health: Math.round(20 * hpMult),
      maxHealth: Math.round(20 * hpMult),
      damage: Math.round(10 * atkMult),
      defense: 5,
    });
  }

  // Stage conditions: roll from TL3 (day 7+), stack at TL5+
  const conditions: StageCondition[] = [];
  if (threatLevel >= 3) {
    const allConditions: StageCondition[] = ['night', 'rain', 'infected_zone', 'fortified', 'fog', 'timed'];
    if (Math.random() < 0.25) {
      const idx = Math.floor(Math.random() * allConditions.length);
      conditions.push(allConditions[idx]);
    }
    // Second condition at TL5+
    if (threatLevel >= 5 && Math.random() < 0.25) {
      const remaining = allConditions.filter(c => !conditions.includes(c));
      if (remaining.length > 0) {
        const idx = Math.floor(Math.random() * remaining.length);
        conditions.push(remaining[idx]);
      }
    }
  }
  ...
  return {
    ...
    conditions: conditions.length > 0 ? conditions : encounter.conditions,
  };
}
...
export const STAGE_2_ENCOUNTERS: Encounter[] = [
  ...
  },
  {
    ...
    description: 'Felt like you were being watched. You were. They drop from fire escapes and emerge from dumpsters. Perception is the only way out.',
    location: 'City Back Streets',
    enemyType: 'ambush',
    enemies: [
      { name: 'Stalker', health: 16, maxHealth: 16, damage: 12, defense: 2 },
      { name: 'Stalker', health: 16, maxHealth: 16, damage: 12, defense: 2 },
      { name: 'Ambush Leader', health: 22, maxHealth: 22, damage: 10, defense: 4 },
    ],
    difficulty: 'medium',
    rewardsText: 'Loot from fallen attackers',
...
export const STAGE_3_ENCOUNTERS: Encounter[] = [
  ...
  {
    id: 'enc_compound_assault',
    stage: 3,
    type: 'combat',
    name: 'Compound Assault',
    description: 'A raider war-band storms the supply depot. Heavy weapons, tactics, radio comms. They know what they\'re doing. So do you.',
    location: 'Supply Depot',
    enemyType: 'raiders',
    enemies: [
      { name: 'Raider', health: 20, maxHealth: 20, damage: 9, defense: 3 },
      { name: 'Raider', health: 20, maxHealth: 20, damage: 9, defense: 3 },
      { name: 'Raider Sergeant', health: 30, maxHealth: 30, damage: 12, defense: 5 },
      { name: 'Raider Heavy', health: 35, maxHealth: 35, damage: 14, defense: 8 },
    ],
    difficulty: 'very_hard',
    rewardsText: 'Seized raider stores',
    itemsToFind: ['card_rifle_001'],
  },
];
...
/**
 * Get a random encounter for a given stage, scaled to the current day.
 */
export function getRandomEncounter(stage: 1 | 2 | 3, day?: number): Encounter {
  const encounters =
  ...
  const base: Encounter = {
    ...encounters[index],
    enemies: encounters[index].enemies?.map(e => ({ ...e })),
  };

  if (day !== undefined && day > 1) {
    const threatLevel = getThreatLevel(day);
    return scaleEncounterToThreat(base, threatLevel);
  }
