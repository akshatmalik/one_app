...
## 2026-06-19 15:30 — Games Tab — Smart Alerts Center + Desktop Notifications

**Files**: app/apps/game-analytics/lib/alerts.ts, app/apps/game-analytics/hooks/useSmartAlerts.ts, app/apps/game-analytics/components/AlertsCenter.tsx, app/apps/game-analytics/page.tsx, UPDATE.md, app/apps/game-analytics/data/whats-new.json
**Risk**: not risky

The app already calculates plenty of "you should know about this" signals — `getSmartNudges()`, `getSpendingForecast()`, `getShelfLifeExpiry()`, `getQueueShameData()` — but each lived in its own corner (a rotating header ticker, a budget chart, individual game cards, queue cards) with no unified, actionable surface, and zero use anywhere in the codebase of the browser's `Notification` API (confirmed via grep — this was a complete gap). Added a new pure aggregator, `getSmartAlerts(games, budgets)` in a new `lib/alerts.ts` (deliberately not touching the existing 14k-line `calculations.ts`), which combines a custom streak-at-risk check (anchored at yesterday so it doesn't double-count via the existing `getCurrentGamingStreak`'s "today must already be logged" behavior), this year's budget risk, the worst 2 shelf-life-expiry games, the worst 1 queue-shame entry, and the top 2 non-streak smart nudges into one `SmartAlert[]` sorted by priority. A new `useSmartAlerts()` hook memoizes that feed and layers in real OS notifications: a localStorage-persisted on/off toggle (scoped per-user like every other device-local preference in this app), a permission-request flow gated on first opt-in, and a same-day dedup log so an urgent alert (priority ≥55 — busted streaks, blown/near budgets) fires a `Notification` once per day rather than every render. `components/AlertsCenter.tsx` is a new bell-icon dropdown (mirroring the existing Command Palette/More-menu pattern in `page.tsx`) showing the full feed with severity-colored cards, a notification on/off control, and a "View game →" action that opens the existing `GameBottomSheet` for any alert tied to a game. Wired into `page.tsx` with one new hook call (`useSmartAlerts(games, budgets, user?.uid)`) and one new header button — no changes to `lib/types.ts`, the storage/repository layer, or any existing `calculations.ts` function. Verified with `npm run build` (11/11 static pages, zero new TypeScript errors) and `npm run lint` (zero new warnings — all pre-existing warnings are in unrelated apps/files); a dev-server smoke test confirmed `/apps/game-analytics` compiles cleanly and serves HTTP 200 with no new console output. No headless browser is available in this environment, so the 375px mobile pass was a manual review of the dropdown's Tailwind layout against the existing More-menu pattern it mirrors.

FOLLOW-UP: Could add a lightweight in-app toast the moment a *new* critical alert appears (even with desktop notifications off), and could let users snooze/dismiss individual alert categories instead of all-or-nothing notification toggling.
